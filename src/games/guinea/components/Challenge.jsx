          ...
          >
            ...
            >
              {mode === 'menu' && (
                <>
                  <h2>⚔️ 친구와 대결!</h2>
                  <p className="challenge-desc">같은 보드로 누가 더 높은 점수를 내는지 겨뤄봐!</p>
                  <div className="challenge-buttons">
                    <button className="btn btn-create" onClick={handleCreate}>🎲 대결 코드 만들기</button>
                    <button className="btn btn-join" onClick={() => setMode('join')}>🔗 코드로 참가하기</button>
                  </div>
                </>
              )}
              {mode === 'create' && (
                <>
                  ...
                  <button className="btn btn-small" onClick={() => { setOpen(false); setMode('menu'); }}>게임 시작하기</button>
                </>
              )}
              {mode === 'join' && (
                <>
                  ...
                  <div className="challenge-input-box">
                    ...
                    />
                    <button className="btn btn-go" onClick={handleJoin}>🐹 도전!</button>
                  </div>
                  <button className="btn btn-small btn-back" onClick={() => setMode('menu')}>← 뒤로</button>
                </>
              )}
            </motion.div>
          </motion.div>
